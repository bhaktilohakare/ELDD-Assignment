...
//step4
	...
	if(IS_ERR(pdevice))
	{
		printk(KERN_ERR "%s: pdevice_create() is failed\n", THIS_MODULE->name);
		ret = -1;
		goto device_create_failed;
	}
	printk(KERN_INFO "%s: pdevice_create() created device file\n", THIS_MODULE->name);
//step5
    ...
    sema_init(&s, 1);
    printk(KERN_INFO "%s: semaphore_init intialized for device.\n", THIS_MODULE->name);
